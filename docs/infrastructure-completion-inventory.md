...
| Cache and provenance | Typed cache, calendar, validation and audit exist | Persist provenance with every experiment record and expose cache decisions | Required | Experiment database | Prior run displays exact source, dates, adjustment and cache action | Run detail/provenance | Claude Code local implementation |
