"""Persistence helpers for robustness evidence artifacts."""
...
def persist_robustness_evidence_artifact(
    *,
    service: Any,
    run_id: str,
    ...
    """Write, register, and manifest one robustness evidence artifact."""
    ...
    artifact = service.register_artifact(
        ...
        location=location,
        content=content,
        schema_version=ROBUSTNESS_EVIDENCE_SCHEMA_VERSION,
    )
    service.persist_run_manifest(service.build_run_manifest(run_id))
    return PersistedRobustnessEvidence(
        artifact=artifact,
        document=document,
...
def retrieve_robustness_evidence_artifact(
    *,
    service: Any,
    run_id: str,
    artifact_root: str | Path,
    ...
    """Retrieve one validated robustness evidence artifact."""
    from persistence import ArtifactType
    ...
    matches = tuple(
        ...
        if artifact.logical_name == ROBUSTNESS_EVIDENCE_LOGICAL_NAME
        and artifact.artifact_type == ArtifactType.VALIDATION_EVIDENCE
    )
    if len(matches) != 1:
        raise ValueError("robustness evidence artifact is missing")
    artifact = matches[0]
    validation_by_id = {
    ...
    if validation is None or not validation.valid or validation.resolved_path is None:
        raise ValueError(
            f"robustness evidence artifact is unavailable: "
    ...
    try:
        ...
        raise ValueError(f"robustness evidence artifact JSON is corrupt: {exc}") from exc
    if not isinstance(document, dict):
        raise ValueError("robustness evidence artifact is not a JSON object")
    validate_robustness_evidence_document(
        document=document,
        run_id=run_id,
        artifact=artifact,
