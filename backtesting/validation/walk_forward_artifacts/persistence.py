"""Persistence helpers for generic walk-forward evidence artifacts."""
...
def persist_walk_forward_evidence_artifact(
    *,
    service: Any,
    run_id: str,
    ...
    """Write, register, and manifest one generic walk-forward evidence artifact."""
    ...
    artifact = service.register_artifact(
        ...
        location=location,
        content=content,
        schema_version=WALK_FORWARD_EVIDENCE_SCHEMA_VERSION,
    )
    service.persist_run_manifest(service.build_run_manifest(run_id))
    return PersistedWalkForwardEvidence(
        artifact=artifact,
        document=document,
...
def retrieve_walk_forward_evidence_artifact(
    *,
    service: Any,
    run_id: str,
    artifact_root: str | Path,
    ...
    """Retrieve one validated generic walk-forward evidence artifact."""
    from persistence import ArtifactType
    ...
    matches = tuple(
        ...
        if artifact.logical_name == WALK_FORWARD_EVIDENCE_LOGICAL_NAME
        and artifact.artifact_type == ArtifactType.VALIDATION_EVIDENCE
    )
    if len(matches) != 1:
        raise ValueError("walk-forward evidence artifact is missing")
    artifact = matches[0]
    validation_by_id = {
    ...
    if validation is None or not validation.valid or validation.resolved_path is None:
        raise ValueError(
            f"walk-forward evidence artifact is unavailable: "
    ...
    try:
        ...
        raise ValueError(f"walk-forward evidence artifact JSON is corrupt: {exc}") from exc
    if not isinstance(document, dict):
        raise ValueError("walk-forward evidence artifact is not a JSON object")
    validate_walk_forward_evidence_document(
        document=document,
        run_id=run_id,
        artifact=artifact,
    )
    _validate_source_lineage(document=document, expected_source=expected_source)
    return PersistedWalkForwardEvidence(
        artifact=artifact,
        document=document,
        evidence_identity=document["artifact"]["evidence_identity"],
