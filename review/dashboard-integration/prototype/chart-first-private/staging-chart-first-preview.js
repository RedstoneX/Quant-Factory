(() => {
  ...
  const EVIDENCE_URL = "/chart-first-private/spym-preview-evidence.json";
  const PAGE_SIZE = 12;
  ...
  function priceTrace(rows, colors) {
    return {
      type: "candlestick", name: `Observed SPYM · ${state.interval}`,
  ...
  async function renderPriceChart() {
    ...
    document.getElementById("chart-counts").textContent = `${aggregatePrices(state.interval).length.toLocaleString()} actual ${state.interval} bars · ${entries.length + exits.length} visible trade markers`;
  ...
  async function load() {
    try {
      ...
      if (evidence.schema_version !== 1 || evidence.instrument !== "SPYM") throw new Error("Preview evidence schema is not recognized.");
      state.evidence = evidence;
      decodeRows(evidence);
      if (state.prices.length !== 53528 || state.trades.length !== 366) throw new Error("Persisted evidence counts do not match the validated preview contract.");
